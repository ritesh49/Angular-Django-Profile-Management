...
export class RegisterComponent implements OnInit {
  ...
  login_user() {
    ...
    if (this.regObj.phone_no != undefined) {
      this.common.login_user(this.regObj.phone_no).subscribe(
        (data) => {
          localStorage.setItem('UserData', JSON.stringify(data));
          this.toaster.showSuccess('User Logged In');
        },
        (err) =>
          this.toaster.showError('User with This Phone No. Does Not Exits'),
  ...
  register_user() {
    ...
    if (this.valid) {
      this.common.registerUser(this.regObj).subscribe(
        (data) => {
          this.user_data = data;
          localStorage.setItem('UserData', JSON.stringify(data));
      ...
      this.toaster.showInfo('Fill the Details Correctly before Submitting');
  }
  ...
  uploadImage() {
    ...
    this.common.uploadImage(image_data, this.user_data.id).subscribe(
      (data) => this.toaster.showSuccess(data['success']),
